...
bench: held CDP. Global warmup + first of each cell discarded. par/waves before seq (idle Chrome stay hot). --repeat counted runs; best_ms=min, median_ms, runs_ms. seq = N Run on one Client. --parallel N = one RunParallel (max worker). --waves W = W RunParallel on those same Clients (no re-lease). No daemon park. Not Jenkins Stage View. Not the app-live pin unless that is --base-url.
